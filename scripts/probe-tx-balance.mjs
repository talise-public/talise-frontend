...
async function probe(amount) {
  ...
  try {
    ...
    console.log(`amount=${amount} BUILD OK · dryRun.status=${JSON.stringify(j.result?.effects?.status ?? j.error)} · gas=${JSON.stringify(j.result?.effects?.gasUsed)}`);
  } catch (e) { console.log(`amount=${amount} BUILD-ERR: ${e.message.slice(0,300)}`); }
}

// Test partial amounts (where current path always fails) + full balance
for (const a of [1000n, 10_000n, 100_000n, 500_000n, 670_716n]) {
  await probe(a);
}
