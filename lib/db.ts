...
async function doEnsureSchema(): Promise<void> {
  ...
  const stmts: string[] = [
    ...
    // `roundup_queue` USED TO BE DECLARED HERE. Spend + Save is now one
    // atomic PTB (transfer + NAVI supply, one signature, one digest), so
    // there is nothing to defer and nothing to drain. The DDL, its helpers
    // and `/api/cron/process-roundup-queue` are gone. Any existing prod
    // table is left in place for /admin's raw browser; nothing reads or
    // writes it. See lib/rewards/roundup.ts.
...
// `enqueueRoundup` / `pendingRoundups` / `markRoundupProcessed` USED TO LIVE
// HERE, feeding `/api/cron/process-roundup-queue`. Spend + Save is one atomic
// PTB now, so the queue, its cron and these helpers are all gone. They had no
// callers left in any case: the gasless legs stopped enqueueing when the save
// became part of the send itself.
