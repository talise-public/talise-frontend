 ...
 * @param {string} root_dec
 * @param {bigint} amount
 * @param {bigint} out0
 * @param {bigint} out1
 * @returns {string}
 */
export function build_deposit_input(pool_hex, root_dec, amount, out0, out1) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(pool_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(root_dec, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.build_deposit_input(ptr0, len0, ptr1, len1, amount, out0, out1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./talise_privacy_circuit_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('talise_privacy_circuit_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
